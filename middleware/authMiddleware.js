...
export const authenticateAndAttachUser = async (req, res, next) => {
  try {
    ...
    if (!authHeader) {
        console.log("❌ [AuthMiddleware] REJECTED: No Authorization Header present.");
        return res.status(401).json({ message: "No token provided" });
    }

    // 2. Check Bearer Format
    if (!authHeader.startsWith("Bearer ")) {
        console.log("❌ [AuthMiddleware] REJECTED: Invalid header format (Missing 'Bearer ').");
    ...
    if (!token || token === "null" || token === "undefined") {
        console.log("❌ [AuthMiddleware] REJECTED: Token string is null/undefined.");
        return res.status(401).json({ message: "No token provided" });
    }
    ...
    jwt.verify(token, secret, async (err, decoded) => {
      if (err) {
        ...
        console.error(`❌ [AuthMiddleware] JWT REJECTED: ${err.name} - ${err.message}`);
      ...
      if (!decoded?.unique_id) {
          console.error("❌ [AuthMiddleware] REJECTED: Token missing 'unique_id'. Payload:", decoded);
      ...
      if (!user) {
          console.log(`❌ [AuthMiddleware] REJECTED: User ${decoded.unique_id} not found in DB.`);
      ...
      if (user.is_banned) {
          if (user.banned_until) {
              ...
              if (new Date() > expiryDate) {
                  ...
                  console.log(`⛔ [AuthMiddleware] REJECTED: User is suspended until ${expiryDate}`);
                  return res.status(403).json({ message: "Account Suspended", reason: user.ban_reason, expires_at: expiryDate });
              }
          } else {
              console.log("⛔ [AuthMiddleware] REJECTED: User is permanently banned.");
